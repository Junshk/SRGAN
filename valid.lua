...
valid_img_fold = 'data/test/BSD100/'
...
input[{{iter}}] = preprocess(image.crop(lr,crop_sx,crop_sy,crop_sx+patch_size,crop_sy+patch_size))
target[{{iter}}] = preprocess(image.crop(gt,crop_sx*scale,crop_sy*scale,crop_sx*scale+patch_size*scale,crop_sy*scale+patch_size*scale))
